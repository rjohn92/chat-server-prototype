...
- **Components:  
  ...
  - **Protocol:** Rules for how data is formatted and exchanged (usually
    > simple: raw text or binary messages).
...
- A simple TCP server that listens for messages from a client and echoes
  > them back

- Focus: Understanding sockets, the TCP lifecycle, and basic
  > client-server interaction

**3.2. Phase 2: Expand to Chat Server**

- Support for multiple clients, message broadcasting, basic session
  > handling, simple authentication

- Focus: Threading, synchronization, resource management, common
  > pitfalls in C

**3.3. Phase 3: Vulnerability Exploration**

- Deliberately introduce, then mitigate, vulnerabilities (buffer
  > overflows, race conditions)

- Analyze compiled binaries (using Ghidra) to link code → assembly →
  > security impact

**3.4. Phase 4: Reverse Engineering and Hardening**

- Use Ghidra to step through **disassembly**. Disassembly is the process
  > of translating a compiled binary back into assembly code
  > (human-readable machine instructions).of our own binaries

- Harden the code, document findings, apply lessons learned to
  > firmware/malware samples
...
- **Development OS:  
  ...
  - Linux (Ubuntu or similar for host/dev; Raspberry Pi for ARM/embedded
    > exposure)
...
- **Recommendation (Best Practice):  
  ...
  - **Option 2:** *LAN Testing*: If you want to connect from other
    > devices, use a separate VLAN or a physically isolated subnet (many
